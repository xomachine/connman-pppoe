...
static DBusMessage *pppoe_get_sec(struct connman_task *task,
				DBusMessage *msg, void *user_data)
{
	...
	user = vpn_provider_get_string(provider, "PPPoE.User");
	passwd = vpn_provider_get_string(provider, "PPPoE.Password");
...
static int pppoe_notify(DBusMessage *msg, struct vpn_provider *provider)
{
	...
	if (strcmp(reason, "auth failed") == 0) {
		...
		vpn_provider_set_string(provider, "PPPoE.User", NULL);
		vpn_provider_set_string(provider, "PPPoE.Password", NULL);
...
static int pppoe_save(struct vpn_provider *provider, GKeyFile *keyfile)
{
	const char *option;
	bool pppoe_option, pppd_option;
	int i;

	for (i = 0; i < (int)ARRAY_SIZE(pppoe_options); i++) {
		pppoe_option = pppd_option = false;

		if (strncmp(pppoe_options[i].cm_opt, "PPPoE", 5) == 0)
			pppoe_option = true;

		if (strncmp(pppoe_options[i].cm_opt, "PPPD.", 5) == 0)
			pppd_option = true;

		if (pppoe_option || pppd_option) {
			...
			if (!option) {
				...
				pppoe_str = g_strdup_printf("PPPoE.%s",
						&pppoe_options[i].cm_opt[5]);
				option = vpn_provider_get_string(provider,
								pppoe_str);
				g_free(pppoe_str);
...
static void pppoe_write_bool_option(struct connman_task *task,
...
{
	struct request_input_reply *pppoe_reply = user_data;
	const char *error = NULL;
	char *username = NULL, *password = NULL;
	char *key;
	DBusMessageIter iter, dict;

	DBG("provider %p", pppoe_reply->provider);
	...
	pppoe_reply->callback(pppoe_reply->provider, username, password, error,
				pppoe_reply->user_data);
			...
			void *user_data)
{
	...
	struct request_input_reply *pppoe_reply;
	int err;
	void *agent;
	...
	vpn_agent_append_user_info(&dict, provider, "PPPoE.User");

	vpn_agent_append_host_and_name(&dict, provider);

	connman_dbus_dict_close(&iter, &dict);

	pppoe_reply = g_try_new0(struct request_input_reply, 1);
	if (!pppoe_reply) {
		dbus_message_unref(message);
		return -ENOMEM;
	}

	pppoe_reply->provider = provider;
	pppoe_reply->callback = callback;
	pppoe_reply->user_data = user_data;

	err = connman_agent_queue_message(provider, message,
			connman_timeout_input_request(),
			request_input_reply, pppoe_reply, agent);
	if (err < 0 && err != -EBUSY) {
			...
			const char *username, const char *password)
{
	...
	for (i = 0; i < (int)ARRAY_SIZE(pppoe_options); i++) {
		opt_s = vpn_provider_get_string(provider,
					pppoe_options[i].cm_opt);
		if (!opt_s)
			opt_s = pppoe_options[i].vpnc_default;

		if (!opt_s)
			continue;

		if (pppoe_options[i].type == OPT_STRING)
			connman_task_add_argument(task,
					pppoe_options[i].pppoe_opt, opt_s);
		else if (pppoe_options[i].type == OPT_BOOL)
	...
	err = connman_task_run(task, vpn_died, provider,
				NULL, NULL, NULL);
	if (err < 0) {
		connman_error("pppoe failed to start");
		err = -EIO;
		goto done;
	}
...
static void free_private_data(struct pppoe_private_data *data)
{
	g_free(data->if_name);
	g_free(data);
}
...
static void request_input_cb(struct vpn_provider *provider,
			...
			const char *error, void *user_data)
{
	struct pppoe_private_data *data = user_data;
	...
	vpn_provider_set_string(provider, "PPPoE.User", username);
	vpn_provider_set_string_hide_value(provider, "PPPoE.Password",
								password);
...
static int pppoe_connect(struct vpn_provider *provider,
			struct connman_task *task, const char *if_name,
			...
			void *user_data)
{
	...
	if (connman_task_set_notify(task, "getsec",
					pppoe_get_sec, provider)) {
		err = -ENOMEM;
		goto error;
	}

	username = vpn_provider_get_string(provider, "PPPoE.User");
	password = vpn_provider_get_string(provider, "PPPoE.Password");

	DBG("user %s password %p", username, password);

	if (!username || !password) {
		struct pppoe_private_data *data;

		data = g_try_new0(struct pppoe_private_data, 1);
		if (!data)
			return -ENOMEM;
...
static int pppoe_error_code(struct vpn_provider *provider, int exit_code)
{
...
static void pppoe_disconnect(struct vpn_provider *provider)
{
	vpn_provider_set_string(provider, "PPPoE.Password", NULL);
}
...
static struct vpn_driver vpn_driver = {
	...
	.error_code     = pppoe_error_code,
	.save		= pppoe_save,
	.disconnect	= pppoe_disconnect,
};
...
static int pppoe_init(void)
{
	...
	return vpn_register("pppoe", &vpn_driver, PPPD);
}

static void pppoe_exit(void)
{
	vpn_unregister("pppoe");

	dbus_connection_unref(connection);
}

CONNMAN_PLUGIN_DEFINE(pppoe, "pppoe plugin", VERSION,
	CONNMAN_PLUGIN_PRIORITY_DEFAULT, pppoe_init, pppoe_exit)
